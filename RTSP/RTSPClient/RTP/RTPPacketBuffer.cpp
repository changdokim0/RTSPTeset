...
int RTPPacketBuffer::ReplayFrameNum()
{
	return m_nFrameNum;
}
	...
	if (fExtension) {
		if (payloadLen() <= 4) {
			...
			if (payloadLen() <= remExtSize) {
				...
				char extraHdr = fCurPtr[0];
				int nSize = extraHdr & 0x0f;
				nSize++;
				if (nSize == 3)
				{
					nExtension = ntohl(*(u_int32_t*)(fCurPtr));
					nExtension = nExtension & 0x00ffffff;
					m_nFrameNum = nExtension;
				}
