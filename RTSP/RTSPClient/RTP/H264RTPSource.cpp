	...
	case 24: {	// STAP-A
		...
		while (len > 3)
		{
			...
			if (fFrameHandlerFunc)
				fFrameHandlerFunc(fFrameHandlerFuncData, fFrameType, media_timestamp, fFrameBuf, fFrameBufPos, packet->ReplayFrameNum());
			resetFrameBuf();
		}
		break;
			 }
...
unsigned char* parseH264ConfigStr(char const* configStr, unsigned int& configSize, unsigned int& spsSize)
{
	...
	if( configStr == NULL || *configStr == '\0' )
		return NULL;
	int nSize = strlen(configStr);
	dup = new char[nSize + 1];
	memset(dup, 0, nSize + 1);
	memcpy(dup, configStr, nSize);
	psz = dup;
