...
public:
	...
	int openURL(const char *url, int streamType, int timeout = 2, bool rtpOnly = false, bool isPlayback = false);
	int openQuickURL(const char* url, int streamType, int timeout = 2, bool rtpOnly = false, bool isPlayback = false);
	...
	int playURL2(FrameHandlerFunc func, void* funcData,
		OnCloseFunc onCloseFunc, void* onCloseFuncData,
		OnPacketReceiveFunc onRTPReceiveFunc = NULL, void* onRTPReceiveFuncData = NULL,
		OnPacketReceiveFunc onRTCPReceiveFunc = NULL, void* onRTCPReceiveFuncData = NULL, std::string start = "", std::string end = "", float scale = 1.0);
	int playQuikURL(FrameHandlerFunc func, void* funcData,
		OnCloseFunc onCloseFunc, void* onCloseFuncData,
		OnPacketReceiveFunc onRTPReceiveFunc = NULL, void* onRTPReceiveFuncData = NULL,
		OnPacketReceiveFunc onRTCPReceiveFunc = NULL, void* onRTCPReceiveFuncData = NULL);
	...
	int sendPlay2(std::string start = "", std::string end = "", float scale = 1.0f);
...
protected:
	...
	bool setupMediaSubsession(MediaSubsession& subsession, 
		...
		bool forceMulticastOnUnspecified = false, bool isPlayback = false);

	bool playMediaSession(MediaSession& session, bool response,
		double start = 0.0f, double end = -1.0f, float scale = 1.0f);

	bool playMediaSession2(MediaSession& session, bool response,
		std::string start = "", std::string  end = "", float scale = 1.0f);

	bool playQuikMediaSession(MediaSession& session, bool response,
		std::string start = "", std::string  end = "", float scale = 1.0f);
