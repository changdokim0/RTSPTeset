...
void RTSPClient::tcpReadHandler1()
{
	...
	case AWAITING_STREAM_CHANNEL_ID: {
		if (lookupStreamChannelId(c)) {
			...
			if (prev_fStreamChannelId != fStreamChannelId) {
				//printf("ch id is : %d\n", fStreamChannelId);
				prev_fStreamChannelId = fStreamChannelId;
			}
			//printf("ch id is : %d\n", fStreamChannelId);

			//if (fStreamChannelId != 0) {
			//	//std::cout << "ch : "<< fStreamChannelId << std::endl;
			//	printf("ch id is not zero: %d\n", fStreamChannelId);
			//}
...
bool RTSPClient::setupMediaSubsession(MediaSubsession& subsession, bool streamOutgoing /* = false */, bool streamUsingTCP /* = false */, 
									  bool forceMulticastOnUnspecified /* = false */, bool isPlayback)
{
	char* cmd = NULL;
	char* setupStr = NULL;
	...
	do 
	{
		...
		std::string send_message = cmd;
		if(isPlayback)
			send_message += "Require: onvif-replay\r\n\r\n";
		else send_message += "\r\n";

		if (!sendRequest((char*)send_message.c_str(), "SETUP"))
			break;
...
static char* createRangeString2(std::string start, std::string end) {
	char buf[300];
	...
	if (start == "") {
		...
		// There's no end time:
		sprintf(buf, "Require: onvif-replay\r\nScale: 1.000\r\nRange: clock=%s-\r\n", start.c_str());
	}
	else {
		// There's both a start and an end time; include them both in the "Range:" hdr
...
bool RTSPClient::parseRTPInfoHeader(char*& line, unsigned short& seqNum, unsigned long long& timestamp)
{
...
bool RTSPClient::parseDateHeader(char const* line, unsigned long long& time)
{
	char* pKey = "Date: ";
	int nSkipLine = strlen(pKey);
	if (_strcasecmp(line, "Date: ", nSkipLine) == 0) {
		line += nSkipLine;
	}
	else if (line[0] == ',') {
		++line;
	...
	std::string format = "%d %b %Y %H:%M:%S GMT";
	...
	if (time == -1)
	{
		...
		ss1 >> std::get_time(&t, format.c_str());
		time = mktime(&t);
	}
								  ...
								  double start, double end, float scale) 
{
	...
	do {
		...
		while (1) {
			...
			unsigned int fTimestampFrequency = 90000;
			if (parseRTPInfoHeader(lineStart, seqNum, timestamp)) {
				...
				while ((subsession = iter.next()) != NULL) {
					...
					fTimestampFrequency = subsession->rtpTimestampFrequency();
			...
			unsigned long long gmtTime = 0;
			if (parseDateHeader(lineStart, gmtTime)) {
				if (fbeginTimeStamp == 0 && timestamp != 0 && gmtTime != 0)
				{
					uint64_t msec = 1000;

					fDateTimeStamp = gmtTime * msec;
					fbeginTimeStamp = timestamp * msec / fTimestampFrequency;
				}
			}
		}
...
bool RTSPClient::playMediaSession2(MediaSession& session, bool response,
	std::string start, std::string end, float scale)
{
	char* cmd = NULL;
	do {
		// First, make sure that we have a RTSP session in progress
		...
		char* scaleStr = createScaleString(scale, session.scale());
		// And then a "Range:" string:
		char* rangeStr = createRangeString2(start, end);
...
bool RTSPClient::playQuikMediaSession(MediaSession& session, bool response,
	std::string start, std::string end, float scale)
{
	char* cmd = NULL;
	do {
	...
	do {
		...
		while ((subsession=iter->next()) != NULL)
		{
			...
			if (streamType == 0)
				...
				res = setupMediaSubsession(*subsession, false, true, false, isPlayback);
...
int RTSPClient::openQuickURL(const char* url, int streamType, int timeout, bool rtpOnly, bool isPlayback)
{
	bool isOpened = false;
	char* username = NULL;
	char* password = NULL;
...
int RTSPClient::playURL2(FrameHandlerFunc func, void* funcData,
	OnCloseFunc onCloseFunc, void* onCloseFuncData,
	OnPacketReceiveFunc onRTPReceiveFunc, void* onRTPReceiveFuncData,
	OnPacketReceiveFunc onRTCPReceiveFunc, void* onRTCPReceiveFuncData, std::string start, std::string end, float scale)
{
	if (!fMediaSession)
		return -1;

	if (!playMediaSession2(*fMediaSession, true, start, end, scale))
		return -1;
...
int RTSPClient::playQuikURL(FrameHandlerFunc func, void* funcData,
	OnCloseFunc onCloseFunc, void* onCloseFuncData,
	OnPacketReceiveFunc onRTPReceiveFunc, void* onRTPReceiveFuncData,
	OnPacketReceiveFunc onRTCPReceiveFunc, void* onRTCPReceiveFuncData)
{
	...
	if (!playQuikMediaSession(*fMediaSession, true))
		return -1;
...
int RTSPClient::sendPlay2(std::string start, std::string end, float scale)
{
	if (!fMediaSession)
		return -1;

	if (!playMediaSession2(*fMediaSession, false, start, end, scale))
		return -1;
